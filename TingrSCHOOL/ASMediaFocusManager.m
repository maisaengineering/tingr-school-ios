...
- (ASMediaFocusController *)focusViewControllerForView:(UIView *)mediaView
{
    ...
    viewController.imageRect = self.imageFrame;
    ...
    [operation setDownloadProgressBlock:^(NSUInteger bytesRead, NSInteger totalBytesRead, NSInteger totalBytesExpectedToRead) {
        ...
        [operation setCompletionBlock:^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                viewController.mainImageView.image = image;
                CGSize size;
                CGRect finalImageFrame = viewController.view.frame;
                size = [self sizeThatFitsInSize:finalImageFrame.size initialSize:image.size];
                finalImageFrame.size = size;
                finalImageFrame.origin.x = (viewController.view.bounds.size.width - size.width)/2;
                finalImageFrame.origin.y = (viewController.view.bounds.size.height - size.height)/2;
                viewController.imageRect = finalImageFrame;
                [viewController.scrollView displayImage:image];
            });
...
- (void)handleFocusGesture:(UIGestureRecognizer *)gesture
{
    ...
    self.imageFrame = finalImageFrame;
    focusViewController.imageRect = finalImageFrame;
...
- (void)handleDefocusGesture:(UIGestureRecognizer *)gesture
{
   ...
   // CGRect __block bounds;
    ...
    //added DM to remove hide animation AND keep the image in the proper position (we can do math and redo control for next release to add back animation)
