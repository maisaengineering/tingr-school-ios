...
- (void)setAttributedText:(NSAttributedString *)attributedText
{
  if (self.mutableAttributedString != attributedText)
  {
    self.mutableAttributedString = [attributedText mutableCopy];
      ...
      /*DM - This set of code is here because we are using a third party control called NIAttributedLabel (to support links, etc) while the comments are using UILabel.
       ...
       So, to offset this, I had to experiment with core text settings and finally had success forcing the leading min and max line spacing
       */
      
      CGFloat minLineHeight = 17.0;
      CGFloat maxLineHeight = 19.5;
      ...
      NSRange range = NSMakeRange(0,self.mutableAttributedString.length);
      
      [self.mutableAttributedString addAttribute:(__bridge id)kCTParagraphStyleAttributeName value:(__bridge id)paragraphStyle range:range];
...
- (void)drawAttributedString:(NSAttributedString *)attributedString rect:(CGRect)rect {
    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        if(self.font == [UIFont fontWithName:@"HelveticaNeue-Light" size:14])
        {
            if(lineIndex > 0)
            {
                CGPoint prevLineOrigin = lineOrigins[lineIndex-1];
                if(prevLineOrigin.y - lineOrigin.y > 17)
                {
                    y += prevLineOrigin.y - lineOrigin.y - 17;
                    lineOrigin.y = prevLineOrigin.y - 17;
